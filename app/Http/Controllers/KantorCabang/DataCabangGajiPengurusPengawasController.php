...
use App\Imports\DataCabangGajiPengurusPengawasImport;
...
class DataCabangGajiPengurusPengawasController extends Controller
{
    public function index($slug)
    {
        ...
        if (!Schema::hasTable('waktus_pengurus_pengawas_' . $tableName)) {
        ...
        $data_waktu = DB::table('waktus_pengurus_pengawas_' . $tableName)
            ->orderBy('id', 'desc')
            ->get();
        return view('dashboard.tambah-kantor.kantor-cabang.gaji-pengurus-pengawas.index', compact('data_waktu', 'tableName', 'years'));
    }
    public function store_waktu(Request $request)
    {
        ...
        $tableName = 'waktus_pengurus_pengawas_' . $request->input('table_name');
    ...
    public function delete_waktus($id_waktu, Request $request)
    {
        ...
        DB::table('pengurus_pengawas_' . $table)->where('id_waktu', $id_waktu)->delete();
    ...
    public function detail($slug, $id)
    {
        ...
        $waktu = DB::table('waktus_pengurus_pengawas_' . $tableName)->where('id', $id)->first();
        $data_gaji = DB::table('pengurus_pengawas_' . $tableName)->where('id_waktu', $id)->get();
        return view('dashboard.tambah-kantor.kantor-cabang.gaji-pengurus-pengawas.detail', compact('tableName', 'waktu', 'data_gaji'));
    }
    ...
    public function detail_gaji($slug, $id, $id_waktu)
    {
        ...
        $karyawan = 'pengurus_pengawas_' . $tableName;
        $waktu = 'waktus_pengurus_pengawas_' . $tableName;
        ...
        return view('dashboard.tambah-kantor.kantor-cabang.gaji-pengurus-pengawas.detail_gaji', compact('tableName', 'gaji', 'id_waktu'));
    }
    ...
    public function import(Request $request)
    {
        ...
        Excel::import(new DataCabangGajiPengurusPengawasImport($id_waktu, $tableName), $file);
    ...
    {
        $tableName = 'pengurus_pengawas_' . $request->input('tableName');
    ...
    {
        $tableName = 'pengurus_pengawas_' . $request->input('table_name');
    ...
    public function print_slip($id, $id_waktu, $table_name)
    {
        ...
        $tableName = 'pengurus_pengawas_' . $table_name;
        $table_name = $table_name;
        $karyawanTable = 'pengurus_pengawas_' . $table_name;
        $waktusTable = 'waktus_pengurus_pengawas_' . $table_name;
        ...
        $pdf = Pdf::loadView('dashboard.tambah-kantor.kantor-cabang.gaji-pengurus-pengawas.print.pdf', [
            'gaji' => $data_gaji,
            'imageSrc' => $imageSrc,
